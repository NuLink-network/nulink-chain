...
```shell
./target/release/nulink-core --help
```
...
template pallet that is [defined in the `pallets`](pallets/utils/src/lib.rs) directory.
...
also replace the default command (`cargo build --release && ./target/release/nulink-core --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/nulink-core --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/nulink-core purge-chain --dev
